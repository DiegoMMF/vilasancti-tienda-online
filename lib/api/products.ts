...
function reshapeProduct(dbProduct: any): Product | undefined {
  ...
  const featuredImage = images.find((img: any) => img.isFeatured) || images[0] || {
...
export async function getProducts({
  ...
  const orderBy = sortKey === 'CREATED_AT' ? { createdAt: reverse ? 'desc' : 'asc' } :
                 sortKey === 'TITLE' ? { title: reverse ? 'desc' : 'asc' } :
                 sortKey === 'PRICE' ? { variants: { price: reverse ? 'desc' : 'asc' } } :
...
export async function getCollectionProducts({
  ...
  const dbProducts = await prisma.product.findMany({
    ...
    orderBy: sortKey === 'CREATED_AT' ? { createdAt: reverse ? 'desc' : 'asc' } :
             sortKey === 'TITLE' ? { title: reverse ? 'desc' : 'asc' } :
             { updatedAt: reverse ? 'desc' : 'asc' }
  });
