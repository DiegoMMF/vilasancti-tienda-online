...
import { collections, products, productVariants, productImages, productCollections } from './schema';
...
export const collectionQueries = {
  ...
  getByHandle: (handle: string) => db.select().from(collections).where(eq(collections.handle, handle)),
  getById: (id: string) => db.select().from(collections).where(eq(collections.id, id)),
};

export const productQueries = {
  getAll: () => db.select().from(products),
  getByHandle: (handle: string) => db.select().from(products).where(eq(products.handle, handle)),
  getById: (id: string) => db.select().from(products).where(eq(products.id, id)),
  getAvailable: () => db.select().from(products).where(eq(products.availableForSale, true)),
...
export const variantQueries = {
  ...
  getAvailable: () => 
    db.select().from(productVariants).where(eq(productVariants.availableForSale, true)),
};
...
import { collections, products, productVariants, productImages, productCollections } from '../lib/db/schema.js';
...
async function verifyMigration() {
  ...
  try {
    ...
    console.log(`Collections: Prisma=${prismaCollections}, Drizzle=${drizzleCollections.length}`);

    const prismaProducts = await prisma.product.count();
    const drizzleProducts = await db.select().from(products);
    console.log(`Products: Prisma=${prismaProducts}, Drizzle=${drizzleProducts.length}`);
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (collection) {
      const collectionData = await db.select().from(collections).where(eq(collections.handle, collection));
      if (collectionData.length === 0) {
        return NextResponse.json({ error: 'Collection not found' }, { status: 404 });
      }
      productsData = await db.select()
        .from(products)
        .innerJoin(productCollections, eq(products.id, productCollections.productId))
